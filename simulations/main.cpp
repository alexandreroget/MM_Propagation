...
int main() {
  ...
  in.dispersion_coefficients.push_back(std::vector<double>({0., 0., 18.938e-3}));
  in.dispersion_coefficients.push_back(std::vector<double>({0., 0., 18.866e-3}));
  ...
  in.nt = nt;
  in.pulse_width = sqrt(beta_2)/(SR*in.nonlinearity_const);
  in.time_window = t_final;

  in.initial_fields = ComplexArraysContainer(nt, 2);
  in.initial_fields[0] = build_soliton(t_final/sqrt(beta_2/2), nt, t0/sqrt(beta_2/2), c*sqrt(2/beta_2), SR*in.nonlinearity_const);
  in.initial_fields[1] = build_soliton(t_final/sqrt(beta_2/2), nt, -t0/sqrt(beta_2/2), -c*sqrt(2/beta_2), 0.8*SR*in.nonlinearity_const);
  
  std::ofstream outputFile("GRIN2_Raman.txt");
  ...
  MultimodePropagation* mm_propagation = new MultimodePropagationCPU_RamanON(in);
  ...
  for(unsigned int i = 0 ; i < nt ; i++) {    
    outputFile << abs(in.initial_fields[0][i]) << " ; " << abs(in.initial_fields[1][i]) << " ; " << abs(Phi_out[0][i]) << " ; " << abs(Phi_out[1][i]) << std::endl;
  }
